...
void Poligono::draw(SDL_Renderer* renderer, Uint8 r, Uint8 g, Uint8 b)
{
    Uint8 before[4];
    ...
    for (int i = 0; i < this->linhas.size(); i++)
        Utils::linhaDDA(renderer, this->pontos[this->linhas[i].first], this->pontos[this->linhas[i].second]);

    SDL_SetRenderDrawColor(renderer, before[0], before[1], before[2], before[3]);
}

void Poligono::fill(SDL_Renderer* renderer, Uint8 r, Uint8 g, Uint8 b)
{
    Uint8 before[4];
    SDL_GetRenderDrawColor(renderer, &before[0], &before[1], &before[2], &before[3]);
    SDL_SetRenderDrawColor(renderer, r, g, b, 0xFF);
    ...
    for (int Yvarredura = Ymin; Yvarredura < Ymax; Yvarredura++)
    {
        ...
        if (intersecoes.size() % 2)
        {
            ...
            {
                Utils::linhaDDA(renderer, Ponto(intersecoes[i], Yvarredura) , Ponto(intersecoes[i+1], Yvarredura));
            }
        } else {
